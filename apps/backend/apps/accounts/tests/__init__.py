"""
Test package for the accounts app.
Existing tests are here; journal-specific tests are in test_journal_service.py.
"""
...
class SeedLedgersTests(TestCase):
    """Tests for seed_outlet_ledgers function."""

    def _make_outlet(self, suffix=''):
        org = Organization.objects.create(
            name=f'Test Org{suffix}', slug=f'test-org{suffix}', plan='pro'
        )
        return Outlet.objects.create(
            organization=org,
            name=f'Test Outlet{suffix}',
            address='1 Main St', city='Mumbai', state='MH',
            pincode='400001',
            gstin=f'27AAPCT1234E1Z{suffix[:1] or "A"}',
            drug_license_no=f'DL-TEST-{uuid.uuid4().hex[:8].upper()}',
            phone='9000000000',
            is_active=True,
        )

    def test_seed_creates_21_groups(self):
        from apps.accounts.management.commands.seed_ledgers import seed_outlet_ledgers
        outlet = self._make_outlet('1')
        # Signal fires on create, so groups already seeded; verify count
        count = LedgerGroup.objects.filter(outlet=outlet).count()
        self.assertEqual(count, 21)

    def test_seed_creates_default_ledgers(self):
        from apps.accounts.management.commands.seed_ledgers import seed_outlet_ledgers
        outlet = self._make_outlet('2')
        count = Ledger.objects.filter(outlet=outlet).count()
        self.assertGreaterEqual(count, 90)

    def test_seed_is_idempotent(self):
        from apps.accounts.management.commands.seed_ledgers import seed_outlet_ledgers
        outlet = self._make_outlet('3')
        count_after_first = LedgerGroup.objects.filter(outlet=outlet).count()
        ledger_count_after_first = Ledger.objects.filter(outlet=outlet).count()

        # Run again — counts must not change
        seed_outlet_ledgers(outlet)
        self.assertEqual(LedgerGroup.objects.filter(outlet=outlet).count(), count_after_first)
        self.assertEqual(Ledger.objects.filter(outlet=outlet).count(), ledger_count_after_first)

    def test_new_outlet_auto_seeds_via_signal(self):
        """Creating an Outlet triggers the post_save signal which calls seed_outlet_ledgers."""
        outlet = self._make_outlet('4')
        groups = LedgerGroup.objects.filter(outlet=outlet).count()
        ledgers = Ledger.objects.filter(outlet=outlet).count()
        self.assertEqual(groups, 21)
        self.assertGreaterEqual(ledgers, 90)

    def test_customer_synced_as_ledger(self):
        from apps.accounts.management.commands.seed_ledgers import seed_outlet_ledgers
        outlet = self._make_outlet('5')
        customer = Customer.objects.create(
            outlet=outlet, name='Test Customer', phone='9111111111'
        )
        seed_outlet_ledgers(outlet)
        ledger = Ledger.objects.filter(outlet=outlet, linked_customer=customer).first()
        self.assertIsNotNone(ledger)
        self.assertEqual(ledger.name, 'Test Customer')

    def test_distributor_synced_as_ledger(self):
        from apps.accounts.management.commands.seed_ledgers import seed_outlet_ledgers
        outlet = self._make_outlet('6')
        distributor = Distributor.objects.create(
            outlet=outlet, name='Test Distributor',
            phone='9222222222', gstin='27AAPCT9999E1Z5',
            address='Dist Road', city='Mumbai', state='MH',
        )
        seed_outlet_ledgers(outlet)
        ledger = Ledger.objects.filter(outlet=outlet, linked_distributor=distributor).first()
        self.assertIsNotNone(ledger)
        self.assertEqual(ledger.name, 'Test Distributor')


class CustomerCreateUpdateTestCase(TestCase):
    """Tests for CustomerListView POST and CustomerDetailView PUT."""
    ...
    def setUp(self):
        ...
        login = self.client.post(
            '/api/v1/auth/login/',
            {'phone': '9111111111', 'password': '1111'},
            format='json',
        )
    ...
    def _create_customer(self, **overrides):
        ...
        return self.client.post('/api/v1/auth/customers/', payload, format='json')

    # ── create ────────────────────────────────────────────────────────────────

    def test_create_customer_minimal(self):
        """Create customer with name and phone only."""
        res = self._create_customer()
        self.assertEqual(res.status_code, 201)
        self.assertEqual(res.data['name'], 'Rahul Verma')
        self.assertEqual(res.data['phone'], '9876543200')
        self.assertFalse(res.data['isChronic'])
        self.assertEqual(res.data['fixedDiscount'], 0.0)
        self.assertEqual(res.data['creditLimit'], 0.0)

    def test_create_customer_all_fields(self):
        """Create customer with all optional fields."""
        res = self._create_customer(
            address='42 Park Lane, Mumbai',
            dob='1990-05-15',
            gstin='27AABCR1234E1Z5',
            isChronic=True,
            fixedDiscount=5,
            creditLimit=10000,
        )
        self.assertEqual(res.status_code, 201)
        self.assertEqual(res.data['gstin'], '27AABCR1234E1Z5')
        self.assertTrue(res.data['isChronic'])
        self.assertEqual(res.data['fixedDiscount'], 5.0)
        self.assertEqual(res.data['creditLimit'], 10000.0)

    def test_create_customer_missing_name(self):
        """Returns 400 when name is missing."""
        res = self._create_customer(name='')
        self.assertEqual(res.status_code, 400)
        self.assertIn('name', res.data['detail'])

    def test_create_customer_missing_phone(self):
        """Returns 400 when phone is missing."""
        res = self._create_customer(phone='')
        self.assertEqual(res.status_code, 400)
        self.assertIn('phone', res.data['detail'])

    def test_create_customer_invalid_phone(self):
        """Returns 400 for non-Indian mobile numbers."""
        for bad_phone in ['1234567890', '5123456789', '98765432', 'abcdefghij']:
            res = self._create_customer(phone=bad_phone)
            self.assertEqual(res.status_code, 400, msg=f'expected 400 for phone={bad_phone}')

    def test_create_customer_valid_phone_formats(self):
        """Accepts all valid Indian mobile prefixes (6-9)."""
        for phone in ['9876543201', '8765432101', '7654321012', '6543210123']:
            Customer.objects.filter(phone=phone).delete()
            res = self._create_customer(phone=phone)
            self.assertEqual(res.status_code, 201, msg=f'expected 201 for phone={phone}')

    def test_create_customer_invalid_gstin(self):
        """Returns 400 for malformed GSTIN."""
        res = self._create_customer(gstin='INVALID')
        self.assertEqual(res.status_code, 400)
        self.assertIn('gstin', res.data['detail'])

    def test_create_customer_invalid_credit_limit(self):
        """Returns 400 for negative creditLimit."""
        res = self._create_customer(creditLimit=-100)
        self.assertEqual(res.status_code, 400)

    def test_create_customer_invalid_fixed_discount(self):
        """Returns 400 for fixedDiscount outside 0-100."""
        res = self._create_customer(fixedDiscount=110)
        self.assertEqual(res.status_code, 400)

    def test_create_customer_outlet_isolation(self):
        """Customer is linked to the correct outlet."""
        res = self._create_customer()
        self.assertEqual(res.status_code, 201)
        customer = Customer.objects.get(id=res.data['id'])
        self.assertEqual(customer.outlet, self.outlet)
    ...
    def test_update_customer(self):
        ...
        url = f'/api/v1/auth/customers/{c.id}/'
        ...
        self.assertTrue(res.data['isChronic'])
        self.assertEqual(res.data['creditLimit'], 5000.0)

    def test_update_customer_invalid_phone(self):
        """PUT returns 400 for bad phone number."""
        c = self._make_customer()
        url = f'/api/v1/auth/customers/{c.id}/'
        res = self.client.put(url, {
            'outletId': str(self.outlet.id),
    ...
    def test_update_customer_invalid_gstin(self):
        """PUT returns 400 for bad GSTIN."""
        c = self._make_customer()
        url = f'/api/v1/auth/customers/{c.id}/'
        res = self.client.put(url, {
            'outletId': str(self.outlet.id),
